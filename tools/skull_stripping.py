...
def reorientation(nii_filepath, outputpath):
    ...
    try:
        ...
        print('Doing reorientation | !!! unable to read this data: ', nii_filepath)

def skull_stripping(nii_files_list, outputpath):
...
def brain(nii_file, output_dir, ncpu=8):
    outputdirs = [os.path.join(output_dir, '1_img'), os.path.join(output_dir, '1_mask'), \
        os.path.join(output_dir, '1_check'), os.path.join(output_dir, '1_err'), os.path.join(output_dir, '0_resample')]
    
    for i in outputdirs:
        mkdirs(i)
    
    print("\n\n")
    print(fixed_length_string(' Skull Stripping ', 100))
    
    print(fixed_length_string(' Reorientation ', 100, padding='-')) # reorientation, RAS+ 1mm^3
    starttime = datetime.datetime.now()
    with multiprocessing.Pool(ncpu) as p:
        p.map(partial(reorientation, outputpath=outputdirs), nii_file)
    now = datetime.datetime.now()
    decorate_print('reorientation and resampling running time:{}s, start time is {}, finish time is {}'.format(((now-starttime).seconds), starttime, now))
    
    print(fixed_length_string(' Deepbrain ', 100, padding='-')) # deepbrain segmentation
    starttime = datetime.datetime.now()
    skull_stripping(nii_file, outputdirs)
    current_time = datetime.datetime.now()
    decorate_print('deepbrain running time:{}s, start time is {}, finish time is {}'.format(((current_time-starttime).seconds), starttime, current_time))